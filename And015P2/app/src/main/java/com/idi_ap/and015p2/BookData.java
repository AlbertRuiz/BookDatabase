...
public class BookData {
    ...
    public Book addBook(String title, String author, int any, String editorial, String categoria, String valoracio) {
        ContentValues values = new ContentValues();
        Log.d("Creating", "Creating " + title + " " + author);
    ...
    public List<Book> getBooksSortedByCategory() {
        List<Book> books = new ArrayList<>();
        Cursor cursor = database.rawQuery("SELECT * FROM books ORDER BY category",null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            Book book = cursorToBook(cursor);
            books.add(book);
    ...
    public List<Book> getBooksSortedByAutor() {
        List<Book> books = new ArrayList<>();
        Cursor cursor = database.rawQuery("SELECT * FROM books ORDER BY author",null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            Book book = cursorToBook(cursor);
            books.add(book);
    ...
    public List<Book> getBooksSortedByTitle() {
        List<Book> books = new ArrayList<>();
        Cursor cursor = database.rawQuery("SELECT * FROM books ORDER BY title",null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            Book book = cursorToBook(cursor);
            books.add(book);
